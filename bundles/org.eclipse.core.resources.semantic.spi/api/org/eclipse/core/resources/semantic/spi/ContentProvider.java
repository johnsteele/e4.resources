...
public abstract class ContentProvider implements ISemanticContentProvider {
	...
	private static final QualifiedName ATTRIBUTE_URI = new QualifiedName(SemanticResourcesSpiPlugin.PLUGIN_ID, "URIString"); //$NON-NLS-1$ 
	private static final QualifiedName CONTENT_TYPE_KEY = new QualifiedName(SemanticResourcesSpiPlugin.PLUGIN_ID, "ContentType"); //$NON-NLS-1$;
	private static final QualifiedName ATTRIBUTE_READONLY = new QualifiedName(SemanticResourcesSpiPlugin.PLUGIN_ID, "readonly"); //$NON-NLS-1$ 
	...
	public IStatus validateRemove(ISemanticFileStore semanticFileStore, int options, IProgressMonitor monitor) throws CoreException {
		return new Status(IStatus.OK, SemanticResourcesSpiPlugin.PLUGIN_ID, ""); //$NON-NLS-1$
	}
