 ...
 * 
 */
public class LockAction extends ActionBase {
		...
		IRunnableWithProgress outerRunnable = new IRunnableWithProgress() {
			...
			public void run(IProgressMonitor outerMonitor) throws InvocationTargetException, InterruptedException {
				...
				for (Iterator it = getSelection().iterator(); it.hasNext();) {
					...
					outerMonitor.subTask(NLS.bind(Messages.LockAction_LockingRes_XMSG, resource.getAdaptedResource().getName()));
					...
					IWorkspaceRunnable wsRunnable = new IWorkspaceRunnable() {
						...
						public void run(IProgressMonitor monitor) throws CoreException {
							IStatus status = resource.lockResource(LockAction.this.options, monitor);
							if (!status.isOK()) {
					...
					try {
						...
						// ISemanticResource.lockResource(IProgressMonitor)
