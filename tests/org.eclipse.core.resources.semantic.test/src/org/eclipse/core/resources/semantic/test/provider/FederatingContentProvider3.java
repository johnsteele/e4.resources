 ...
 * 
 */
public class FederatingContentProvider3 extends ContentProvider implements ISemanticContentProviderFederation2 {

	private static final Map<String, String> pathTemplatMap = new HashMap<String, String>();

	static {
		pathTemplatMap.put("B", FederatedContentProvider.class.getName());
		// TODO use another content provider
		pathTemplatMap.put("C", "org.eclipse.core.resources.semantic.provider.DefaultContentProvider");

		// invalid ID
		pathTemplatMap.put("D", "invalid ID");
	}
	...
	public FederatedProviderInfo getFederatedProviderInfoForPath(IPath path) {
		...
		if (checkPath.segment(0).equals("X")) {
			if (pathTemplatMap.get(checkPath.segment(1)) != null) {
				return new FederatedProviderInfo(pathTemplatMap.get(checkPath.segment(1)), 2);
			}
		}
		return null;
	}
