	...
	public void synchronizeContentWithRemote(ISemanticFileStore semanticFileStore, SyncDirection direction, IProgressMonitor monitor,
		...
		if (semanticFileStore.getType() == ISemanticFileStore.FILE) {
			...
			try {
				...
				// $JL-EXC$ ignore
				status.add(new Status(IStatus.ERROR, SemanticResourcesPlugin.PLUGIN_ID, e.getMessage()));
				return;
			...
			if (direction == SyncDirection.BOTH) {
				...
				long localTimestamp;
				try {
					localTimestamp = getCacheService().getContentTimestamp(semanticFileStore.getPath());
				} catch (CoreException e) {
					status.add(e.getStatus());
					return;
				}
			...
			if (direction == SyncDirection.OUTGOING || syncOut) {
				try {
					...
					file.setLastModified(getCacheService().getContentTimestamp(semanticFileStore.getPath()));
				} catch (CoreException e) {
					status.add(e.getStatus());
				} catch (IOException e) {
					// $JL-EXC$ ignore
