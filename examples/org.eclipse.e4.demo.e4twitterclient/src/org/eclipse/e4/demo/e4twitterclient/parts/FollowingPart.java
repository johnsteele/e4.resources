...
public class FollowingPart implements IUserRefresh {
	...
	@Inject
	public FollowingPart(Composite parent, final IWorkspace workspace) {
		this.workspace = workspace;
		this.project = workspace.getRoot().getProject(TwitterContentUtil.PROJECT_NAME); //$NON-NLS-1$

		this.tableFriends = createTable(parent);
	}
	...
	public void refreshUser(String userName) {
		...
		try {
			if (userName != null) {
				users = TwitterContentUtil.getFriends(this.project, userName);
			} else {
				users = null;
			}
				...
				public void run() {
					...
					if (users != null) {
						for (UserType user : users) {
							TableItem item = new TableItem(tableFriends, SWT.NONE);
							item.setText(0, user.getScreenName());
							item.setText(1, user.getName());
						}
					}
					for (int i = 0; i < titles.length; i++) {
						tableFriends.getColumn(i).pack();
					}
				}
			});
